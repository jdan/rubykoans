...
class AboutRegularExpressions < Neo::Koan
  ...
  # NOTE(jdan): They take up as many letters as they can
  ...
  def test_a_vertical_pipe_means_or
    ...
    # NOTE(jdan): Oh interesting, the syntax is [/regexp/, idx]
  ...
  # NOTE(jdan): `|` define different words while character classes operate on
  ...
  def test_sub_is_like_find_and_replace
    # NOTE(jdan): This appears to replace "two" with just... t? Because of [0, 1]
    assert_equal "one t-three", "one two-three".sub(/(t\w*)/) { $1[0, 1] }
  end

  def test_gsub_is_like_find_and_replace_all
    # NOTE(jdan): Oh I see, gsub will replace all of them
